...
function SellPageInner() {
  ...
  useEffect(() => {
    ...
    supabase.from('categories').select('name').order('sort_order')
      .then(({ data }) => { if (data) setCategories(data.map((c: {name: string}) => c.name)) })
  }, [campusSlug])
    ...
    <>
      <style suppressHydrationWarning>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .page-title { font-family: 'Syne', sans-serif; font-weight: 800; font-size: 32px; color: #E8EEFF; margin-bottom: 8px; letter-spacing: -0.02em; }
        ...
        .input { width: 100%; padding: 12px 16px; border: 1px solid rgba(123,92,240,0.18); border-radius: 12px; font-size: 14px; font-family: 'Space Grotesk', sans-serif; background: #111125; color: #E8EEFF; outline: none; transition: border-color 0.15s, box-shadow 0.15s; }
        ...
        .price-prefix { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); font-size: 15px; font-weight: 600; color: #8892A4; font-family: 'IBM Plex Mono', monospace; }
        .price-input { padding-left: 30px !important; }
        .price-type-toggle { display: flex; gap: 8px; margin-top: 10px; }
        .price-type-btn { flex: 1; padding: 10px 12px; border-radius: 10px; border: 1px solid rgba(123,92,240,0.2); background: transparent; font-size: 13px; font-weight: 500; cursor: pointer; font-family: 'Space Grotesk', sans-serif; transition: all 0.15s; display: flex; align-items: center; justify-content: center; gap: 6px; color: #8892A4; }
        .price-type-btn.type-fixed.active { background: rgba(255,255,255,0.07); border-color: rgba(255,255,255,0.4); color: #E8EEFF; }
        .price-type-btn.negotiable.active { background: rgba(234,179,8,0.1); border-color: rgba(234,179,8,0.5); color: #facc15; }
        .char-count { font-size: 11px; color: #5A6480; text-align: right; margin-top: 5px; font-family: 'IBM Plex Mono', monospace; }
        .cat-grid { display: flex; gap: 8px; flex-wrap: wrap; }
        .cat-btn { padding: 10px 18px; border-radius: 100px; border: 1px solid rgba(123,92,240,0.18); background: transparent; font-size: 13px; font-weight: 500; color: #8892A4; cursor: pointer; transition: all 0.15s; font-family: 'Space Grotesk', sans-serif; min-height: 40px; }
        ...
        .upload-zone { border: 2px dashed rgba(123,92,240,0.25); border-radius: 16px; padding: 24px; text-align: center; cursor: pointer; transition: all 0.15s; background: #111125; }
        ...
        .preview-img { width: 90px; height: 90px; object-fit: cover; border-radius: 10px; display: block; border: 1px solid rgba(123,92,240,0.18); }
        .preview-remove { position: absolute; top: -8px; right: -8px; width: 28px; height: 28px; border-radius: 50%; background: #FF2D9B; color: white; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; }
        .cover-badge { position: absolute; bottom: 4px; left: 4px; background: rgba(0,212,255,0.9); color: #06060F; font-size: 9px; font-weight: 700; padding: 2px 6px; border-radius: 4px; letter-spacing: 0.05em; font-family: 'IBM Plex Mono', monospace; }
        .phone-box { background: rgba(123,92,240,0.06); border: 1px solid rgba(123,92,240,0.18); border-radius: 12px; padding: 14px 16px; }
        ...
        .submit-btn { width: 100%; padding: 16px; background: linear-gradient(135deg, #7B5CF0, #5B3FD0); color: white; border: none; border-radius: 12px; font-size: 16px; font-weight: 600; font-family: 'Space Grotesk', sans-serif; cursor: pointer; transition: all 0.15s; margin-top: 8px; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 0 24px rgba(123,92,240,0.3); }
        ...
        @media (max-width: 600px) { .navbar { padding: 14px 16px; } .page { padding: 24px 16px 60px; } .input { font-size: 16px; padding: 14px 16px; } .upload-zone { padding: 20px 16px; } .page-title { font-size: clamp(24px, 7vw, 32px); } }
      `}</style>
      ...
      <div className="page">
        ...
        <div className="field">
          ...
          <input className={`input ${errors.title ? 'error' : ''}`} placeholder="e.g. MTech Textbook Set" value={title} onChange={e => setTitle(e.target.value)} />
        ...
        <div className="field">
          ...
          <div className="price-wrap">
            ...
            <input type="number" className={`input price-input ${errors.price ? 'error' : ''}`} placeholder="500" value={price} onChange={e => setPrice(e.target.value)} min="0" />
          </div>
          {errors.price && <div className="error-text">{errors.price}</div>}
          <div className="price-type-toggle">
            <button type="button" className={`price-type-btn type-fixed${priceType === 'fixed' ? ' active' : ''}`} onClick={() => setPriceType('fixed')}>💰 Fixed price</button>
            <button type="button" className={`price-type-btn negotiable${priceType === 'negotiable' ? ' active' : ''}`} onClick={() => setPriceType('negotiable')}>🤝 Negotiable</button>
          </div>
        </div>

        <div className="field">
          <label className="label">Product age *</label>
          <select className={`input ${errors.productAge ? 'error' : ''}`} value={productAge} onChange={e => setProductAge(e.target.value)}>
            <option value="">Select age</option>
            {PRODUCT_AGES.map(age => <option key={age} value={age}>{age}</option>)}
          </select>
        ...
        <div className="field">
          ...
          <textarea className={`input ${errors.description ? 'error' : ''}`} placeholder="Describe condition, reason for selling, etc." value={description} onChange={e => setDescription(e.target.value.slice(0, 300))} />
        ...
        <div className="field">
          ...
          {images.length < 3 && (
            <div className={`upload-zone ${errors.images ? 'error' : ''}`} onClick={() => fileInputRef.current?.click()}>
            ...
            <div className="previews">
              {previews.map((src, i) => (
                <div className="preview-wrap" key={i}>
                  ...
                  <button className="preview-remove" onClick={() => removeImage(i)}><X size={12} /></button>
                </div>
              ))}
            </div>
          )}
...
export default function SellPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#06060F' }} />}>
      <SellPageInner />
    </Suspense>
  )
}